...
def cli(argv: Optional[List[str]] = None) -> None:
    ...
    p_extract = sub.add_parser(
        "extract", help="Run full feature extractor (wraps extract_features_full)"
    )
    p_extract.add_argument(
        "--input", required=True, help="Input folder with preprocessed .fif files"
    )
    p_extract.add_argument(
        "--out", required=True, help="Output folder for feature files"
    )
    p_extract.add_argument(
        "--window", type=float, default=10.0, help="Window length (s)"
    )
    p_extract.add_argument(
        "--overlap", type=float, default=0.5, help="Window overlap fraction"
    )
    p_extract.add_argument(
        "--per-channel", action="store_true", help="Include per-channel features"
    )
    p_extract.add_argument(
        "--features",
        type=str,
        default=None,
        help="Comma-separated list of feature names to enable (defaults to all)",
    )
    p_extract.add_argument(
        "--save-spectrograms",
        action="store_true",
        help="Save spectrogram tensors if available",
    )
    p_extract.add_argument(
        "--save-connectivity",
        action="store_true",
        help="Save connectivity matrices if available",
    )
