...
def _fake_spectrogram(
    n_channels: int = 2, n_freq: int = 40, n_time: int = 20
) -> np.ndarray:
...
def test_extract_and_sample_patches_and_vq() -> None:
    ...
    S = _fake_spectrogram()
    patches = extract_patches_from_spectrogram(
        S, freq_bins=5, time_bins=4, stride_freq=2, stride_time=2
    )
    assert len(patches) > 0

    sampled = sample_patches_from_epochs(
        [S], n_patches=8, freq_bins=5, time_bins=4, seed=1
    )
